...
from app.models import (
    ExerciseResponse as ExerciseResponseModel, Exercise, UserProgress, UserStats,
    ExerciseSubmit, ExerciseSubmitResponse, UserProgressDetailResponse, UserStatsResponse, UserProgressResponse
)
...
async def submit_exercise(
    ...
    response = ExerciseResponseModel(
    ...
    if stats:
        ...
        user_responses = db.query(ExerciseResponseModel).filter(ExerciseResponseModel.user_id == user_id).all()
...
@router.get("/user/{user_id}", response_model=UserProgressDetailResponse)
async def get_user_progress(user_id: int, db: Session = Depends(get_db)):
    ...
    return UserProgressDetailResponse(
        stats=UserStatsResponse.from_orm(stats),
